...
def main(param=None):
    ...
    digits and spaces. If no argument is provided, the program prompts the
    user to enter a string interactively.
    """
    try:
        if param is None:
            print("What is the text to count ?")
            # fct input("texte_d'amorce") pour récup le texte écrit sur stdin
            inp = input(">> ")
            param = inp
        ...
        # (1 for char in param if char.isupper()) => générateur qui produit 1
        # pour chaque caractère char dans la chaîne param qui satisfait la
        # condition char.isupper() (c'est-à-dire les majuscules).
        ...
        n_punctuation = sum(
            1 for char in param if not (char.isalnum() or char.isspace())
        )
