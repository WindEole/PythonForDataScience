...
# SHUTIL
# 	sert pour l'affichage dynamique de la barre en fonction
# 	de la largeur de la ligne

# TQDM
# 	tqdm (taqadum en arabe, signifier progresser, ou avancer) est une biblio
# 	Python qui permet de créer des barres de progression pour les boucles.
...
def ft_tqdm(lst: range) -> None:
    ...
    # on obtient la largeur du terminal
    ...
    def print_progress(lst):
        ...
        # 3.0 => 3 char avant la virgule, comblé par des espaces,
        # et ne pas afficher les décimales
        percent = f"{100 * (lst / float(total)):>3.0f}"
        suffix = (
            f"{lst:>3.0f}/{total} "
            f"[{elapsed_str}<{remaining_str}, {speed:.2f}it/s]"
        )
        # calcul de la longueur deja remplie :
        filled_len = int(
            (shell_width - len(percent) - len(suffix) - 4) * lst // total
        )
        # calcul de la longueur restante :
        bar = fill * filled_len + " " * (
            (shell_width - len(percent) - len(suffix) - 4) - filled_len
        )

        # 4 characteres / espaces en plus
        print(f"\r{percent}%|{bar}| {suffix}", end="\r")
        if lst == total:
            print()
