...
# 	Convertit un n-uplet ou struct_time représentant une heure renvoyée
# 	par gmtime() ou localtime() en une chaîne spécifiée par l’argument format.
...
if __name__ == "__main__":
    if len(sys.argv) != 1:
        ...
        date_ns = format(date_sec, ".2e")  # On veut sa notation scientifique
        # (format = builtin python !)
        # |-> arrondi à 4 decimales
        print(
            f"Seconds since January 1, 1970: {round(date_sec, 4):,}"
            f" or {date_ns} in scientific notation",
        )
